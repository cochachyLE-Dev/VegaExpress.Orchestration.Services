...
using static VegaExpress.Agent.Controls.GridView;
...
namespace VegaExpress.Agent.Controls
{
    public class GridView : View
    {
        ...
        public int Top
        {
            ...
            set
            {
                top = TableIsNullOrInvisible() ? 0 : Math.Min(Table.Rows.Count - 1, Math.Max(0, value));
            }
        }
        /// <summary>
        ...
        public GridView(DataTable table) : this()
        {
            Table = table;
        }
        ...
        private void RenderRow(int row, int rowToRender, ColumnToRender[] columnsToRender)
        {
            ...
            for (int i = 0; i < columnsToRender.Length; i++)
            {
                ...
                Terminal.Gui.Attribute cellColor;
                if (isSelectedCell)
                {
        ...
        protected virtual void RenderCell(Terminal.Gui.Attribute cellColor, string render, bool isPrimaryCell)
        {
        ...
        private void RenderCell(int col, int row, string render)
        {
            if (row < 0 || col < 0)
                return;
            if (row > base.Frame.Height - 1 || col > base.Frame.Width - 1)
                return;
        ...
        public void PageUp(bool extend)
        {
            ...
            int n = selectedRow - Frame.Height;
            if (n < 0)
                n = 0;
            if (n != selectedRow)
            {
                ...
                SetSelection(FullRowSelect ? SelectedColumn : 0, selectedRow, extend);
                SetNeedsDisplay();
            }
        ...
        {
            var n = selectedRow + Frame.Height;
            if (n > table.Rows.Count)
                n = table.Rows.Count - 1;
            if (n != selectedRow)
            {
                selectedRow = n;
                if (table.Rows.Count >= Frame.Height)
                    top = selectedRow;
                else
                    top = 0;
        ...
        private bool IsColumnVisible(int columnIndex)
        {
            ...
            return Style.GetColumnStyleIfAny(Table.Columns[columnIndex])?.Visible ?? true;
        }
        ...
        public override void PositionCursor()
        {
            ...
            if (screenPoint != null)
                Move(screenPoint.Value.X, screenPoint.Value.Y - top);
        }
        ...
        public override bool MouseEvent(MouseEvent me)
        {
            ...
            switch (me.Flags)
            {
                case MouseFlags.WheeledDown:
                    ...
                    while (selectedRow < RowOffset)
                    {
                        ++selectedRow;
                        SetSelection(0, selectedRow, me.Flags.HasFlag(MouseFlags.ButtonShift));
                        SetNeedsDisplay();
                ...
                case MouseFlags.WheeledUp:
                    ...
                    SetSelection(0, RowOffset, me.Flags.HasFlag(MouseFlags.ButtonShift));
                    SetNeedsDisplay();
        ...
        void MoveDown(MouseEvent me)
        {
            if (selectedRow + 1 < table.Rows.Count)
            {
                selectedRow++;
                if (selectedRow >= top + Frame.Height)
                    top++;
                SetSelection(0, selectedRow, me.Flags.HasFlag(MouseFlags.ButtonShift));
                SetNeedsDisplay();
            }
    ...
    public class GridView<T> : GridView
    {
        private List<T> _items = new List<T>();
        private List<Column> _columns = new List<Column>();
        private List<ColumnRestriction> _columnRestrictions = new List<ColumnRestriction>();

        public T this[int index] => _items.Count >= index ? _items[index] : default(T)!;

        private DataTable _table = new DataTable();

        private int MaxLengthColumn { get; set; }
        private int MaxLengthRow { get; set; }
        public int MaxLength { get => Math.Max(MaxLengthColumn, MaxLengthRow); }        

        public GridView() { }
        public GridView(IEnumerable<T> items)
        {
            _items = items.ToList();
            UpdateView();
        }
        ...
        {
            var propertyPaths = _columns.Select(x => x.Name).ToArray()!;

            if (!(_columns.All(c => _table.Columns.Contains(c.Caption!)) && _columns.Count == _table.Columns.Count))
            {
                _table.Columns.Clear();
                foreach (Column column in _columns)
                {
                    _table.Columns.Add(column.Caption);
                }
            }
            ...
            foreach (var item in _items)
            {
                ...
                foreach (Column column in _columns)
                {
                    foreach (var expression in column.GetExpression!)
                    {
                        if(column.IsExpression)
                            dataRow[column.Caption!] = ExpressionExtention.CompileGetExpression(column.Expression!, item!) ?? DBNull.Value;
                        else
                            dataRow[column.Caption!] = ExpressionExtention.CompileGetExpression(expression!, item!) ?? DBNull.Value;
                    }
                }
        ...
        public void SetDataSource(IEnumerable<T> items)
        {
            _items = items.ToList();
            UpdateView();
        }

        public TResult Max<TResult>(Func<T, TResult> selector) => _items.Max(selector)!;
        ...
        public void AddColumns(Expression<Func<T, object>> columnSelector)
        {
            ...
            int index = 0;
            foreach (var property in ExpressionExtention.GetLambdaExpression(columnSelector))
            {
                Column column = new Column(property.FieldName, property.FieldName, property.Path, 100, index++, true);
                column.GetExpression = property.GetExpression;
                column.SetExpression = property.SetExpression;

                MaxLengthColumn = Math.Max(column.Length, MaxLengthColumn);
                _columns.Add(column);
            }
        }
        public void AddColumn(Expression<Func<T, object>> columnSelector)
        {
            var property = ExpressionExtention.GetLambdaExpression(columnSelector).First();

            Column column = new Column(property.FieldName, property.FieldName, property.Path, 100, _columns.Count, true);
            column.GetExpression = property.GetExpression;
            column.SetExpression = property.SetExpression;
        ...
        public void AddColumn(Expression<Func<T, object>> columnSelector, string? caption, int width = 100, int visibleIndex = 0, bool visible = true, bool isUpdateable = true)
        {
            var property = ExpressionExtention.GetLambdaExpression(columnSelector).First();

            Column column = new Column(property.FieldName, caption, property.Path, width, visibleIndex == 0 ? _columns.Count : visibleIndex, visible);
            column.GetExpression = property.GetExpression;
            column.SetExpression = property.SetExpression;
        ...
        public void AddColumn(Expression<Func<T, object>> columnSelector, Action<Column> columnConfigurator)
        {
            var property = ExpressionExtention.GetLambdaExpression(columnSelector).First();
        ...
        public void AddColumnExpression(Expression<Func<T, object>> columnSelector, string? caption, int width = 100, int visibleIndex = 0, bool visible = true)
        {
            var property = ExpressionExtention.GetLambdaExpression(columnSelector).First();

            Column column;
            if (property.FieldName == "Expression")
            {
                string fieldName = string.Format("Expression_{0}", _columns.Count(x => x.Name!.StartsWith("Expression_")));
                column = new Column(fieldName, caption, property.Path, width, visibleIndex == 0 ? _columns.Count : visibleIndex, visible);
                column.Expression = columnSelector;                
            }
            else
            {
                column = new Column(property.FieldName, caption, property.Path, width, visibleIndex == 0 ? _columns.Count : visibleIndex, visible);
            }
            column.GetExpression = property.GetExpression;
            column.SetExpression = property.SetExpression;
        ...
        public void AddRestriction(Expression<Func<T, object>> columnSelector, RestrictionType type)
        {
            // Get the property
            var memberExpression = columnSelector.Body as MemberExpression;
            if (memberExpression == null && columnSelector.Body is UnaryExpression unaryExpression)
            {
            ...
            if (property == null)
            {
                throw new ArgumentException("Selector must select a property.");
            }

            var restriction = new ColumnRestriction(property.Name, type);

            _columnRestrictions.Add(restriction);
        }
        ...
        public class ColumnStyleSetter
        {
            ...
            public Func<object, TextAlignment>? AlignmentGetter { get; set; }
            public CellColorGetterDelegate? ColorGetter { get; set; }
            public Func<object, string>? RepresentationGetter { get; set; }

        }
        public void SetColumnStyle(Expression<Func<T, object>> selector, Action<ColumnStyle> setter)
        {
            // Get the property
            var memberExpression = selector.Body as MemberExpression;
            if (memberExpression == null && selector.Body is UnaryExpression unaryExpression)
            {
                memberExpression = unaryExpression.Operand as MemberExpression;
            }
            var property = memberExpression?.Member as PropertyInfo;
            ...
            var captionName = _columns.Single(x => x.Name!.Equals(property.Name)).Caption;
            Style.ColumnStyles.Add(_table.Columns[captionName!]!, columnStyle);

            Application.MainLoop.Invoke(Update);
        }

        public void UpdateRow(T newRow)
        {
            var restrictions = _columnRestrictions.Where(x => x.Type == RestrictionType.ID).Select(x => typeof(T).GetProperty(x.FieldName!));
            var existingRowIndex = _items.FindIndex(x => restrictions.All(property => property!.GetValue(x)!.Equals(property.GetValue(newRow))));

            if (existingRowIndex >= 0)
                _items[existingRowIndex] = newRow;
            UpdateView();
        }
        ...
        {
            var restrictions = _columnRestrictions.Where(x => x.Type == RestrictionType.ID).Select(x => typeof(T).GetProperty(x.FieldName!));
            var existingRowIndex = _items.FindIndex(x => restrictions.All(property => property!.GetValue(x)!.Equals(property.GetValue(row))));
            if (existingRowIndex >= 0)
            {
                _items.RemoveAt(existingRowIndex);
        ...
        public void AddOrUpdateRow(T row)
        {
            var restrictions = _columnRestrictions.Where(x => x.Type == RestrictionType.ID).Select(x => typeof(T).GetProperty(x.FieldName!));
            var existingRowIndex = _items.FindIndex(x => restrictions.All(property => property!.GetValue(x)!.Equals(property.GetValue(row))));
        ...
        public void AddOrUpdateRow(T row, Expression<Func<T, object>> columnSelector)
        {
            var restrictions = _columnRestrictions.Where(x => x.Type == RestrictionType.ID).Select(x => typeof(T).GetProperty(x.FieldName!));
            var existingRowIndex = _items.FindIndex(x => restrictions.All(property => property!.GetValue(x)!.Equals(property.GetValue(row))));
            ...
            if (existingRowIndex >= 0)
            {
                var newRow = _items[existingRowIndex];
                foreach (var property in ExpressionExtention.GetLambdaExpression(columnSelector))
                {
                    for (int i = 0; i < property.GetExpression.Length; i++)
                    {
                        var value = ExpressionExtention.CompileGetExpression(property.GetExpression[i], row);
                        ExpressionExtention.CompileSetExpression(property.SetExpression[i], newRow, value);                        
                    }
                }
                _items[existingRowIndex] = newRow;
            }
        ...
        {
            return _items.IndexOf(key);
        }
        public int GetIndexOf(Func<T, bool> predicate)
        {
            return _items.IndexOf(_items.FirstOrDefault(predicate)!);
        }
    ...
    {
        public ColumnRestriction(string? fieldName, RestrictionType? type)
        {
            FieldName = fieldName;
            Type = type;
        }
    ...
    public class Column
    {
        public Column(string? name, string? path) : this(name, name, path, 100, 0, true) { }
        public Column(string? name, string? caption, string? path, int width, int visibleIndex, bool visible)
        {
            Name = name;
            Caption = caption;
            Path = path;
        ...
        public bool IsExpression { get => Expression != null; }
        public LambdaExpression? Expression { get; set; }
        public LambdaExpression[]? GetExpression { get; set; }
        public LambdaExpression[]? SetExpression { get; set; }
    }
}
